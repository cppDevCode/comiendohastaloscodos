document.addEventListener('DOMContentLoaded', () => {
    ...
    async function addDish(dish) {
        ...
        const response = await fetch('https://pascalizado.pythonanywhere.com/platos', {
            headers: {
    ...
    async function addPrice(precios) {
        ...
        let ultimoID
        await fetch('https://pascalizado.pythonanywhere.com/precios?getultimoid=1')
        .then ((res) => res.json())
        ...
        const response = await fetch('https://pascalizado.pythonanywhere.com/precios', {
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json'
              },
    ...
    async function updateDish(id, dish) {
        ...
        plato.tipo = dish.get("tipo")        
        plato = JSON.stringify(plato)
        const response = await fetch(`https://pascalizado.pythonanywhere.com/platos?editarid=${id}`, {
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json'
              },
    ...
    async function updatePrice(id, precio) {
        ...
        const response = await fetch(`https://pascalizado.pythonanywhere.com/precios?editarid=${id}`, {
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json'
              },
    ...
    async function deleteDish(id) {
        await fetch(`https://pascalizado.pythonanywhere.com/platos?borrarid=${id}`, {
            method: 'DELETE'
        });
        await fetch(`https://pascalizado.pythonanywhere.com/precios?borrarid=${id}`, {
            method: 'DELETE'
        });
        loadDishes();
    }
    ...
    async function loadDishes() {
        const response = await fetch('https://pascalizado.pythonanywhere.com/platos?traertodos=1');
        dishes = await response.json();
        let precios = []
        await fetch('https://pascalizado.pythonanywhere.com/precios?traertodos=1')
        .then((res) => res.json())
        .then ((data) => 
        {
